		...
		if(downloadurl.length > 0) {
			...
			https.get(downloadurl, (resp) => {
				if(resp.statusCode == 200) {
					...
					resp.on('data', (chunk) => { fs.appendFileSync('resources/' + addonname + '.part', chunk) })
					resp.on('end', () => {
						spawn('cmd', ['/C', 'ping 127.0.0.1 -n 2 > NUL & del "' + self.package_infos[addonname].file.replace(/\//g, '\\') + '" & copy resources\\' + addonname + '.part resources\\' + addonname + '.asar & del resources\\' + addonname + '.part & "' + process.execPath + '"'], {'detached': true})
						app.quit()
					})
				} else {
					app.quit()
