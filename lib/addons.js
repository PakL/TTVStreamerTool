...
class Addons extends UIPage {
	...
	static get waitForExitSnippet() {
		return '@echo off\r\n:LOOP\r\ntasklist | find /i "TTVStreamerTool" >nul 2>&1\r\nIF ERRORLEVEL 1 (\r\nGOTO CONTINUE\r\n) ELSE (\r\ntimeout /T 1 /Nobreak >nul\r\nGOTO LOOP\)\r\n:CONTINUE\r\n'
	}
	...
	addonclick(addonname) {
		if(this.addonInstalled(addonname)) {
			if(this.package_infos[addonname].file.endsWith('.asar')) {
				...
				modal.onclick = () => {}
				setTimeout(() => {
					fs.writeFileSync('deleteAddon.bat', Addons.waitForExitSnippet + 'del "' + this.package_infos[addonname].file.replace(/\//g, '\\') + '"\r\nstart "" "' + process.execPath + '"\r\nexit')
		...
		if(downloadurl.length > 0) {
			...
			https.get(downloadurl, (resp) => {
				if(resp.statusCode == 200) {
					...
					resp.on('data', (chunk) => { fs.appendFileSync('resources/' + addonname + '.part', chunk) })
					resp.on('end', () => {
						fs.writeFileSync('updateAddon.bat', Addons.waitForExitSnippet + 'del "' + this.package_infos[addonname].file.replace(/\//g, '\\') + '"\r\ncopy resources\\' + addonname + '.part resources\\' + addonname + '.asar\r\ndel resources\\' + addonname + '.part\r\nstart "" "' + process.execPath + '"\r\nexit')
