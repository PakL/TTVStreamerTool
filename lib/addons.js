...
class Addons {
	...
	addonclick(addonname) {
		if(this.addonInstalled(addonname)) {
			if(this.package_infos[addonname].file.endsWith('.asar')) {
				let modal = this.tool.ui.showErrorMessage(new Error('Uninstalling ' + addonname + ' now... Program will restart.'))
				modal.onclick = () => {}
				setTimeout(() => {
				...
				this.tool.ui.showErrorMessage(new Error('Cannot uninstall. For uninstall the addon must be a asar addon.'))
			}
		} else {
			let downloadurl = ''
			for(let i = 0; i < this.available_addons.length; i++) {
			...
			if(downloadurl.length > 0) {
				let modal = this.tool.ui.showErrorMessage(new Error('Downloading and installing ' + addonname + ' now... Program will restart.'))
				modal.onclick = () => {}
				...
				this.tool.ui.showErrorMessage(new Error('Could not find anything to download.'))
			}
		}
	}
	...
	updateclick() {
		...
		if(downloadurl.length > 0) {
			let modal = this.tool.ui.showErrorMessage(new Error('Downloading and updating ' + addonname + ' now... Program will restart.'))
			modal.onclick = () => {}
