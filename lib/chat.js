...
class Chat {
	constructor(username, authtoken) {
		...
		this.irc.on('autohostingyou', (to, viewers, msg) => {
			if(openChannelObject.hasOwnProperty('name') && to == openChannelObject.name) {
				self.showmsg('', '', to, msg, self.usertags, 2)
				var hostmessage = i18n.__('is auto hosting the channel')
				if(viewers > 0) {
					hostmessage = i18n.__('is auto hosting the channel with {{viewernum}} {{viewers||viewernum}}', { viewernum: viewers })
				}
				self.actionsElement._tag.addAction(usr, hostmessage, timestamp(new Date().getTime(), true))
			}
		})
	...
	showmsg(prefix, user, to, msg, tags, type) {
		...
		if(type == 0) {
			for(var i = 0; i < __highlights.length; i++) {
				if(__highlights[i].regex) {
					try {
						var r = new RegExp(__highlights[i].phrase, (__highlights[i].casesensitive ? '' : 'i'))
					...
					if(
						(__highlights[i].casesensitive && org_msg.indexOf(__highlights[i].phrase) >= 0) ||
						(!__highlights[i].casesensitive && org_msg.toLowerCase().indexOf(__highlights[i].phrase.toLowerCase()) >= 0)
					) {
						type = 5
						break;
					}
