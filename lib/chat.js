...
class Chat extends EventEmitter {
	constructor(tool) {
		...
		this.irc.on('reconnect', () => {
			if(self.tool.cockpit.openChannelId.length > 0) {
				self.showmsg('', '', self.tool.cockpit.openChannelObject.login, self.tool.i18n.__('Connection to TMI was lost. Reconnection attempt in 3 seconds...'), {}, 1)
			}
		})
	...
	openViewercard(channel, user)
	{
		...
		viewerCard.webContents.on('did-finish-load', () => {
			viewerCard.webContents.executeJavaScript('document.querySelector("body").classList.add("tw-root--theme-dark");document.querySelector(".tw-root--theme-light").classList.remove("tw-root--theme-light");')
		})
