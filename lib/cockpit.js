...
class Cockpit extends UIPage {
	...
	onHostingyou(channel, user, viewers, msg, tags) {
		...
		if(viewers > 0) {
			...
			;(async function() {
				try {
					let stream = await self.tool.twitchhelix.getStreams({ user_login: user.user, first: 1 })
					if(stream !== null && typeof(stream.data) !== 'undefined' && stream.data.length > 0) {
						let game_id = stream.data[0].game_id
						let game = await self.tool.twitchhelix.getGames(game_id)
						if(game !== null && typeof(game.data) !== 'undefined' && game.data.length > 0) {
							self.channelActionsElement._tag.addAction(user, self.i18n.__('was playing {{game}} last', { game: game.data[0].name }), timestamp(new Date().getTime(), true))
						}
					}
				} catch(e) {}
			})()
