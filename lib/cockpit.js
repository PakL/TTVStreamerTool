		...
		this.tool.on('load', () => {
			...
			self.tool.chat.on('usernotice', (channel, user, tags, msg) => {
				...
				if(tags['msg-id'] == 'subgift') {
					if(!self.tool.settings.showSubscriptionAlert) return
					self.channelActionsElement._tag.addAction(user,  self.i18n.__('gifted {{recipient}} a {{plan}} subscription', {recipient: tags['msg-param-recipient-display-name'], plan: tags['msg-param-sub-plan-name']}), timestamp(parseInt(tags['tmi-sent-ts']), true))
				}
				if(tags['msg-id'] == 'raid') {
					...
					if(badges != null && badges.hasOwnProperty('badge_sets')) {
						...
						self.tool.twitchapi.getCheermotes(channelid, (cheermotes, error) => {
							...
							if(cheermotes != null && cheermotes.hasOwnProperty('actions')) {
								...
								console.dir(cheermotes.actions)
