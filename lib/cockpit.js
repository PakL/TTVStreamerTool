...
class Cockpit extends UIPage {
	...
	leaveChannel() {
		...
		this.tool.chat.part(this.openChannelObject.login)
		this.openChannelId = ''
		this.isChannelOnline = true
		this.openChannelObject = {}
		this.prevMinute = -1
		...
		document.querySelector('#stream_video').innerHTML = ''
	...
	setupVideoplayer() {
		...
		const self = this
		this.pausePlayerTimeout = null
		this.mediasPlaying = 0
		videoPlayer.onclick = () => {
			videoPlayer.onclick = () => {}
			let webview = document.createElement('webview')
			webview.setAttribute('src', 'http://player.twitch.tv/?channel=' + self.openChannelObject.login + '&allowfullscreen=false&muted=true')
			...
			webview.addEventListener('media-started-playing', () => {
				...
				self.mediasPlaying++
				clearTimeout(self.pausePlayerTimeout)
			})
			webview.addEventListener('media-paused', () => {
				console.log('Media paused')
				clearTimeout(self.pausePlayerTimeout)
				self.mediasPlaying--
				if(self.mediasPlaying <= 0) {
					self.pausePlayerTimeout = setTimeout(() => {
