...
class Cockpit extends UIPage {
	...
	async loadMoreFollows(reset) {
		...
		if(follows != null && follows.hasOwnProperty('data')) {
			...
			self.followChannels.sort((a, b) => {
				if(a.id == self.userDataObject.id) return -1
				if(b.id == self.userDataObject.id) return 1
			...
			if(total <= self.followChannels.length) {
				self.followsElement._tag.hideButton()
			}
		} else {
			self._ui.showErrorMessage(new Error(self.tool.i18n.__('Unexpected response')))
		}
	...
	onRoomstate(channel, tags) {
		if(channel != this.openChannelObject.login) return
		let modes = []
		if(typeof(tags['emote-only']) && tags['emote-only'] == '1') modes.push(this.i18n.__('Emotes only'))
		if(typeof(tags['followers-only']) && tags['followers-only'] == '1') modes.push(this.i18n.__('Followers only'))
		if(typeof(tags['slow']) && tags['slow'] == '1') modes.push(this.i18n.__('Slow mode'))
		if(typeof(tags['subs-only']) && tags['subs-only'] == '1') modes.push(this.i18n.__('Subscribers only'))
