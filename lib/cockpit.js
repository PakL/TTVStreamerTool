...
class Cockpit extends UIPage {
	...
	constructor(toolui) {
		...
		this.tool.on('load', () => {
			...
			self.channelOpenInputElement.onkeyup = async function(e) {
				if(e.which == 13) {
					self._ui.startLoading()
					let usr = null
					try {
						usr = await self.tool.twitchapi.getUserByName(self.channelOpenInputElement.value)
					} catch(err) {
					...
					if(usr != null && usr.hasOwnProperty('users')) {
						if(usr.users != null && usr.users.length > 0) {
							self.openChannel(usr.users[0]._id)
						} else {
							self._ui.showErrorMessage(new Error(self.tool.i18n.__('No channel with this name was found.')))
						}
					} else {
						self._ui.showErrorMessage(new Error(self.tool.i18n.__('Unexpected response')))
					}
				}
			}
			...
			document.querySelector('#channel_status').addEventListener('keyup', async (e) => {
				if(e.which == 13 && self.openChannelId.length > 0) {
					e.target.disabled = true
					try {
						let response = await self.tool.twitchapi.updateChannel(self.openChannelId, { status: e.target.value })
			...
			document.querySelector('#channel_game').addEventListener('keyup', async (e) => {
				if(e.which == 13 && self.openChannelId.length > 0) {
					e.target.disabled = true
					try {
						let response = await self.tool.twitchapi.updateChannel(self.openChannelId, { game: e.target.value })
						if(response != null && response.hasOwnProperty('game')) {
							e.target.value = response.game
						}
					} catch(error) {
						self._ui.showErrorMessage(error)
					}
			...
			self.tool.chat.once('userstate', async (channel, user, tags) => {
				if(channel != self.openChannelObject.name) return
				self.userselement._tag.joinusr(user)
				if(typeof(tags['emote-sets']) == "string") {
					try {
						self.emoticons_data = await self.tool.twitchapi.getChatEmoticonsBySet(tags['emote-sets'])
						self.emoticons._tag.setemotes(self.emoticons_data)
					} catch(e) {
						self._ui.showErrorMessage(e)
					}
				}
			})
			...
			self.tool.chat.on('usernotice', (channel, user, tags, msg) => {
				...
				if(tags['msg-id'] == 'resub') {
					...
					self.channelActionsElement._tag.addAction(user,  self.i18n.__('subscribed with {{plan}} for the {{months}}. month in a row!', {plan: plan, months: tags['msg-param-months']}), timestamp(parseInt(tags['tmi-sent-ts']), true))
				}
				if(tags['msg-id'] == 'sub') {
					if(!self.tool.settings.showSubscriptionAlert) return
					self.channelActionsElement._tag.addAction(user,  self.i18n.__('subscribed with {{plan}} to this channel', {plan: plan}), timestamp(parseInt(tags['tmi-sent-ts']), true))
				}
				if(tags['msg-id'] == 'subgift') {
					if(!self.tool.settings.showSubscriptionAlert) return
					self.channelActionsElement._tag.addAction(user,  self.i18n.__('gifted {{recipient}} a {{plan}} subscription', {recipient: tags['msg-param-recipient-display-name'], plan: plan}), timestamp(parseInt(tags['tmi-sent-ts']), true))
				}
				if(tags['msg-id'] == 'raid') {
			...
			self.tool.chat.on('cheer', (channel, ts, user, bits) => {
				if(!self.tool.settings.showCheerAlert) return
				let cheermessage = self.i18n.__('just cheered with {{num_bits}} {{bits||num_bits}}', {num_bits: bits})
				self.channelActionsElement._tag.addAction(user, cheermessage, ts)
			})
	...
	open() {
		...
		if(!this.tool.settings.showViewerList) {
			...
			document.querySelector('#chat_message_emotes').style.left = 'calc(70% - 280px)'
		} else {
			...
			document.querySelector('#chat_message_emotes').style.left = 'calc(50% - 280px)'
		}
	}
	...
	async loadMoreFollows(reset) {
		...
		if(reset) {
			...
			let chnl = null
			try {
				chnl = await this.tool.twitchapi.getChannel()
			} catch(err) {
				self._ui.showErrorMessage(err)
				return
			}
			...
			self._ui.stopLoading()
			if(chnl != null && chnl.hasOwnProperty('_id')) {
				self.followChannels.push(chnl)
		...
		let follows = null
		try {
			follows = await this.tool.twitchapi.getUserFollows('', {'offset': this.followsOffset, 'sortby': 'last_broadcast', 'limit': (this.followChannels.length == 1 ? 39 : 40)})
		} catch(err) {
			self._ui.showErrorMessage(err)
			return
		}
	...
	async openChannel(channelid) {
		...
		try {
			self.openChannelObject = await this.tool.twitchapi.getChannel(channelid)
			self.tool.chat.channelcheers = (await this.tool.twitchapi.getCheermotes(channelid)).actions
		} catch(error) {
		...
		try {
			...
			self.tool.chat.channelbadges = (await this.tool.twitchapi.getChatBadgeSetsByChannel(channelid)).badge_sets
		} catch(error) {
			this._ui.showErrorMessage(error)
		}
		...
		if(self.openChannelObject.hasOwnProperty('name')) {
			// Join IRC channel
