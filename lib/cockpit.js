...
class Cockpit extends UIPage {
	...
	hookEvents() {
		...
		/* Chat stuff */
		this.tool.chat.once('userstate',	(c, u, t)	=> { self.onUserstate(c, u, t) })
		...
		this.tool.chat.on('chatmessage',	(c, ts, u, m, mr, t)=> { self.onChatmessage(c, ts, u, m, mr, t) })
	...
	async loadMoreFollows(reset) {
		...
		if(reset) {
			...
			if(chnl != null && chnl.hasOwnProperty('data') && chnl.data.length > 0) {
				self.userDataObject = chnl.data[0]
				self.followersCursor = null
				self.followChannels.push(chnl.data[0])
		...
		let follows = null
		try {
			let userFollows = await this.tool.twitchhelix.getUsersFollows(self.userDataObject.id, '', (this.followChannels.length == 1 ? 39 : 40), self.followersCursor)
		...
		if(follows != null && follows.hasOwnProperty('data')) {
			for(var i in follows.data) {
				self.followChannels.push(follows.data[i])
			}
	...
	leaveChannel() {
		...
		this.userselement._tag.clearUsers()
	...
	async updateChannel(input, data) {
		input.disabled = true
		try {
			let response = await self.tool.twitchapi.updateChannel(self.openChannelId, data)
			if(response != null) {
		...
		this.channelActionsElement._tag.addAction(
			{
				...
				color: this.tool.chat.userselement._tag.getUserColor(this.openChannelObject.name)
			},
	...
	onFollow(user, raw) {
		if(this.tool.settings.showFollowAlert) {
			this.channelActionsElement._tag.addAction(user, this.tool.i18n.__('is following this channel now'), timestamp(raw.created_at, true))
		}
	}

	async onUserstate(channel, user, tags) {
		if(this.emotesLoaded) return
		if(channel != this.openChannelObject.name) return
	...
	onUsernotice(channel, user, tags, msg) {
		...
		if(tags['msg-id'] == 'resub') {
			...
			this.channelActionsElement._tag.addAction(user,  this.i18n.__('subscribed with {{plan}} for the {{months}}. month in a row!', {plan: plan, months: tags['msg-param-months']}), timestamp(parseInt(tags['tmi-sent-ts']), true))
		}
		if(tags['msg-id'] == 'sub') {
			if(!this.tool.settings.showSubscriptionAlert) return
	...
	onChatmessage(channel, ts, user, message, msg_raw, type) {
		if(channel != this.openChannelObject.name) {
		...
		if(user.user.length > 0 && type == 0 && channel == this.openChannelObject.name) {
			this.userselement._tag.joinusr(user)
		}
	}
