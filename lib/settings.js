...
class ToolSettings extends UIPage {
	...
	async cloudAuthenticate()
	{
		...
		try {
			let response = await request.post('https://sync.ttvst.app/', {json: true, body: {'action': 'authenticate', 'token': this._tool.twitchhelix.token } })
	...
	async cloudCompare(token)
	{
		...
		try {
			let response = await request.post('https://sync.ttvst.app/', {json: true, body: {'action': 'compare', 'token': token, 'lastsynctime': timestamp } })
	...
	async cloudPush(token, data, iv)
	{
		...
		try {
			let response = await request.post('https://sync.ttvst.app/', {json: true, body: {'action': 'push', 'token': token, 'data': data, 'iv': iv } })
	...
	async cloudPull(token)
	{
		...
		try {
			let response = await request.post('https://sync.ttvst.app/', {json: true, body: {'action': 'pull', 'token': token } })
			return response
		} catch(e) {
			if(typeof(e.error) !== 'undefined' && typeof(e.error.status) !== 'undefined') {
				if(e.error.status == 403) {
