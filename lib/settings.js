...
class ToolSettings extends UIPage {
	...
	async syncCloud() {
		...
		if(token.length > 0) {
			let cmp = await this.cloudCompare(token)
			if(cmp !== null) {
				if(cmp.matching) {
					let d = this.cloudPackData()
					await this.cloudPush(token, d[0], d[1])
				} else {
					let userInp = await this.cloudOverwriteDialog(cmp.newer)
					if(userInp) {
						let resp = await this.cloudPull(token)
