let timestamp = function(ts, returnday){
	...
	var ret = ((d.getHours() < 10) ? '0' : '') + d.getHours() + ':' + ((d.getMinutes() < 10) ? '0' : '') + d.getMinutes()

	if(typeof(returnday) == 'boolean' && returnday)
		ret = (d.getDate() < 10 ? '0' : '') + d.getDate() + '.' + (d.getMonth()+1 < 10 ? '0' : '') + (d.getMonth()+1) + '.' + d.getFullYear() + ' ' + ret
...
let getColor = function(name) {
	...
	var c = defaultColors[n % defaultColors.length][1]
