...
var replaceEmoticons = function(text, emotes, cheermotes, bits){
		...
		for(var i = 0; i < e.length; i++) {
			...
			if(splits.length == 2) {
				...
				for(var j = 0; j < ranges.length; j++) {
					...
					if(indexes.length == 2)
						replacings.push({ 'replaceWith': '<img src="http://static-cdn.jtvnw.net/emoticons/v1/'+eid+'/1.0" srcset="http://static-cdn.jtvnw.net/emoticons/v1/'+eid+'/1.0 1x, http://static-cdn.jtvnw.net/emoticons/v1/'+eid+'/2.0 2x, http://static-cdn.jtvnw.net/emoticons/v1/'+eid+'/3.0 4x" alt="{__NEEDLE__}" title="{__NEEDLE__}" class="emote">', 'start': parseInt(indexes[0]), 'end': parseInt(indexes[1]) })
				}
			}
		}
	...
	while(match = regex.exec(text)) {
		//console.dir(match)
		var url = match[2]
		if(typeof(match[3]) == 'undefined' || match[3].length == 0) url = 'http://' + url
